startGame = function() {
  ...
  let player = { height: 30, width: 30, x: 100, y: 480, vx: 0, vy: 0 };
  let _player = { height: 30, width: 30, x: 100, y: 480, vx: 0, vy: 0 };
  let obstacles = [];
  let backgroundObs = [];
  let foregroundObs = [];
  const friction = 0.75;
    ...
    for (var i = 0; i < obstacles.length; i++) {
      if (checkCollision(player, obstacles[i]) == true) {
        if (obstacles[i].reset) {
          player = Object.assign(player, obstacles[i].reset);
        } else if (
          player.vy > 0 &&
